...
class NeuralNet:
    def __init__(self,
                 ...
                 hidden_node_biases=None,
                 output_node_biases=None,
                 weights=None):
        self.neurons = []
        self.weights = []
        ...
        self.set_hidden_neurons(hidden_node_biases)
        self.set_output_neurons(output_node_biases)
    ...
    def set_hidden_neurons(self, hidden_node_biases):
        i = 0
        for layer_number in range(self.number_of_layers - 2):
            layer = []
            for neuron_position in range(self.number_of_neurons_per_layer):
                bias = hidden_node_biases[i] if hidden_node_biases else random.random()
                i += 1
                neuron = Neuron(bias, layer_number + 2, neuron_position)
                layer.append(neuron)
            self.neurons.append(layer)

    def set_output_neurons(self, output_node_biases):
        layer = []
        for neuron_position in range(self.number_of_output_neurons):
            bias = output_node_biases[neuron_position] if output_node_biases else random.random()
    ...
    def train(self, input_data, output_data, epochs=10000):
        ...
        for epoch in range(epochs):
            for x, y in zip(input_data, output_data):
                ...
                #     if y[i] == 1:
                #         correct_index = i
                # correct_output_neuron_activation = self.get_output_layer()[correct_index].get_activation()
                ...
                #         new_weight_value = sigmoid_derivative(output_neuron.get_sum()) * \
